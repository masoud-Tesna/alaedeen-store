...
import { Col, Row, Collapse, Statistic, Skeleton } from "antd";
    ...
    <Row className="mt-4 about--container">
      ...
      {isLoading ?
        ...
        <>
              ...
              <Row>
                <Col span={24} className="mb-5">
                  <Row>
                    ...
                    {aboutStores?.company_introduction?.fields?.detailed_company_introduction &&
          ...
          {(aboutStores?.general?.business_type || aboutStores?.basic_company_details?.fields?.year_company_registered || aboutStores?.basic_company_details?.fields?.total_no_employees || aboutStores?.basic_company_details?.fields?.main_category (aboutStores?.general?.country || aboutStores?.general?.state) || aboutStores?.export_capability?.fields?.main_markets_and_distribution?.length) &&
            ...
            <Row>
              ...
              <Col span={24} className="mt-3 about--componentOverview__tableSection">
                <Row className="row-cols-2 row-cols-md-4 " gutter={16}>
                  ...
                  {
                    aboutStores?.export_capability?.fields?.main_markets_and_distribution?.length ?
                      <>
                    ...
                    <Col span={24} className="mt-3 about--videosSection">
                      <div className="about--videos">
                        { Object.entries(aboutStores?.manufacturing_capability?.parents?.process?.fields)?.map(([key, process], index) => {
                          if (process?.process_pictures.length !== 0 || process?.process_name) {
                            return (
                              <div key={ `manufacturing_capability_process_${ index }` } className="about--video">
                                <Link to={ `/about/manufacturing/?slide=${ key }` }>
                                  <div className="rounded-10 about--imageBg"
                                       style={ { backgroundImage: `url(${ process.process_pictures[ 0 ] })` } }/>
                                  <div
                                    className="text-white ml-4 mb-4 vv-font-size-1-8 font-weight-600 about--imageBg__title">
                    ...
                    <Col span={24} className="mt-3 about--imagesSection">
                      <div className="about--images">
                        { Object.entries(aboutStores?.certificate_center?.parents?.certification?.fields)?.map(([key, certificate], index) => {
                          if (certificate?.certificate_photos.length !== 0 || certificate?.certificate_name) {
                            return (
                              <div key={ `manufacturing_capability_process_${ index }` } className="about--image">
                                <Link to={ `/about/certificate/?slide=${ key }` }>
                                  <Row>
                                    ...
                                    <Col className="m-0 p-0 vv-font-size-1-6 text-center text-47 mt-3"
                                         xs={ 24 }>{ certificate?.certificate_name }</Col>
                                  </Row>
                                </Link>
                              </div>
                            );
